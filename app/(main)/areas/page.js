    ...
    async function fetchAreas() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.length > 0) {
            // Process the data to parse towns JSON if needed
            const processedAreas = data.map(area => ({
              ...area,
              towns: typeof area.towns === 'string' ? JSON.parse(area.towns) : area.towns
            }));
    ...
    <>
      ...
      <section style={{ padding: '80px 0', backgroundColor: '#F1F3E0' }}>
        <div className="container">
          ...
          {loading ? (
            <LoadingSpinner text="Loading areas..." variant="default" size="medium" />
          ) : (
